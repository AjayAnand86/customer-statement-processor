...
public class SwaggerConfig {
  ...
  public Docket api() {
    return new Docket(DocumentationType.SWAGGER_2).select()
        .apis(RequestHandlerSelectors.basePackage("nl.rabobank.customerstatementprocessor"))
        .paths(PathSelectors.regex(".*")).build().apiInfo(apiMetadata());
  }
  ...
  private ApiInfo apiMetadata() {
    return new ApiInfoBuilder().title("Customer statement processor")
        .description("This project processes customer statements and generated reports.").build();
  }
}
